<template>
    <div class="component-container">
      ...
      <select v-model="selectedRocket">
        <option v-for="rocket in rockets" :key="rocket" :value="rocket[0]">{{ rocket[1] }}</option>
      </select>
    ...
    methods: {
      async getRockets() {
        try {
         ...
         // this.rockets = data.map(rocket => rocket.id);
         this.rockets = data.map( function (rocket){
          return [rocket.id, rocket.name];
         });
         // this.height = data.map(rocket => rocket.height);
          //this.rockets = data.map(rocket => rocket.height);
        } catch (error) {
